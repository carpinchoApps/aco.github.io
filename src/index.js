...
if(newChain) {
	...
	document.querySelector('BODY').innerText = 'No se pasaron los parámetros o no ingresaste un enlace válido.'
}
...
const createDatabase = (dbname, newChain) => {
	let a = newChain.split('|')[0], b = parseInt(c.de(newChain.split('|')[1])), d = c.de(newChain.split('|')[3]), db
	const constApp = {Chain : a, LimiteMaximoDeCoopACargar : b, user : d}
	const req = indexedDB.open(dbname,1)
...
const updateConst = (dbname, newChain) => {
	let a = newChain.split('|')[0], b = parseInt(c.de(newChain.split('|')[1])), d = c.de(newChain.split('|')[3])
	const data = {Chain : a, LimiteMaximoDeCoopACargar : b, user: d}
	const req = indexedDB.open(dbname,1)
	req.onsuccess = () => {
